...
Tout d'abord, lorsqu'une personne tente de s'inscrire une fonction ajax vérifie que l'adresse mail entrée n'a jamais été utilisée si elle l'a déjà été, la fonction vérifie que l'adhérent n'est pas déjà cotisant donc déjà inscrit. Si c'est le cas elle retourne un message d'erreur :

\begin{verbatim}
if ($adresse){ 
	SELECT Count(*) from Adherent Where adresse_mail_adherent='".$adresse."';
	....
	....
	if($result==0){
		Message = OK ! 
	else{
		"SELECT count(c.id_adherent) from Adherent a, Cotisation c where adresse_mail_adherent='".$adresse."'and c.id_adherent=a.id_adherent";
		....
		....
		if ($result==0){
			message ="attention cette adresse mail est utilisé pour un compte en attente de cotisation. 
		else {
			message ="adresse mail déja utilisé pour un autre compte";
		}
\end{verbatim}
